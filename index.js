angular.module("app", ["chart.js"]).controller("AppController", function($scope) {
    ...
    $scope.calculateDividends = function(rate, start, payoutMode, overallGrowthRate) {
        ...
        if (numPeriod != 0) {
            ...
            if (payoutMode == "reinvest") {
                var reinvestArray = [];
                reinvestArray.push(array[0]);
                for (var i = 1; i < array.length; i++) {
                    reinvestArray[i] = reinvestArray[i - 1] + array[i];
                    reinvestArray[i] = reinvestArray[i] * (1 + overallGrowthRate / 100);
                }
                for (var i = 0; i < numPeriod; i++) {
                    reinvestArray[i] = reinvestArray[i] + $scope.cashValue[i];
                }
                // create another line chart
                $scope.data[1] = reinvestArray;
                $scope.datasetOverride[1] = {
                    label: "Dividends Reinvested into Fund",
                    borderWidth: 1,
                    type: 'bar'
                };
    ...
    $scope.calculateDividends = function(rate, start, payoutMode, overallGrowthRate) {
        ...
        if (numPeriod != 0) {
            ...
            if (payoutMode == "reinvest") {
                ...
                for (var i = 0; i < numPeriod; i++) {
                    reinvestArray[i] = Math.round(reinvestArray[i] + $scope.cashValue[i]);
                }
                // create another line chart
                $scope.data[1] = reinvestArray;
                $scope.datasetOverride[1] = {
